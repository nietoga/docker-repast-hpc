...
void RepastHPCModel::initHumans(){
	...
	for (int i=0; i<countOfHumans; i++){
		...
		int age=ageInitializer(repast::Random::instance()->nextDouble());
...
void RepastHPCModel::initGridValueLayers(){
	...
	for (int i = 0; i < xdim; i++) {
		for (int j = 0; j < ydim; j++) {
			...
			//initialize temperature- TEMPORAL 
			int temperature= repast::Random::instance()->createUniIntGenerator(16,22).next();//CAMBIAR (TEMPORAL)
...
//update gridValueLayers each day
void RepastHPCModel::runAllPatches(){
	...
	for (int i = 0; i < xdim; i++) {
		for (int j = 0; j < ydim; j++) {
			...
			//update number of suceptible, exposed and infected mosquitoes
...
void RepastHPCModel::runAllHumans(){
	...
	for(int activityNum=0;activityNum<=1;activityNum++){
		it=humans.begin();
		if(activityNum==0){std::cout << "movimiento a other \n";}
		else{std::cout<<"moviemiento a school/work  \n";}
		while(it!=humans.end()){
...
void RepastHPCModel::initSchedule(repast::ScheduleRunner& runner){
	...
	//runner.scheduleEndEvent(repast::Schedule::FunctorPtr(new repast::MethodFunctor<RepastHPCModel> (this, &RepastHPCModel::recordResults))); //this event is schedulled to run after the simulation arrives at the stop end
...
int RepastHPCModel::ageInitializer(double prob) {
	if(0<=prob && prob<0.0811){ //los numeros de distribuciones se crean asi
		IntUniformGenerator UniformGen=repast::Random::instance()->createUniIntGenerator(5, 10);
		return UniformGen.next();
	}
