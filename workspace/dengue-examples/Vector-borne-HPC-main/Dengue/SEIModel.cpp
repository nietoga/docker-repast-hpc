...
void SEIModel::actualizeTemp(int tick,std::vector<std::vector<int>> dataTemperatures){ ////TEMPORAL: se deben leer los datos del file con las temperaturas diaras
    	int maxTemp=dataTemperatures[0][tick];
		int minTemp=dataTemperatures[1][tick];
		int temperature= repast::Random::instance()->createUniIntGenerator(minTemp,maxTemp).next();
}
